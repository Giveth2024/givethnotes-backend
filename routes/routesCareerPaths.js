...
router.get('/career-paths/:id', async (req, res) => {
  try {
    const user_id = await getUserIdFromRequest(req); // authenticated user
    const { id } = req.params;
    ...
    const [results] = await db.query(sql, [user_id, id]);
...
router.put('/career-paths/:id', async (req, res) => {
  try {
    ...
    const [result] = await db.query(sql, [title, description || null, image_url || null, updated_at, id, user_id]);
